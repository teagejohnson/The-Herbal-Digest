...
const RemediesPage = () => {
    ...
    useEffect(() => {
        onSnapshot(query(collection(db, 'remedies'), where('slug', '==', slug)), (snapshot) => {
    ...
    return (
        <>
            ...
            <div>
                {width >= 750 ?
                    <Box style = {{width: '100vw', height: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', alignItems: 'center', backgroundImage: `url(${data.image})`, backgroundSize: 'cover', backgroundPosition: 'center'}}>
                        <Box style = {{width: '100vw', height: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', alignItems: 'center', backgroundColor: data.image ? 'rgba(0, 0, 0, 0.50)': '#171717', backdropFilter: 'blur(5px)'}}>
                            <Box style = {{width: '100vw', height: '85vh', display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', color: 'white'}}>
                                <Box style = {{width: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'justify'}}>
                                    <Fade in timeout = {1600}>
                                        <Typography variant = 'h1' fontFamily = 'Segoe UI, sans-serif' fontWeight = 'regular' align = 'center' sx = {{textShadow: '0px 0px 10px black'}}>
                                            {data.title}
                                        </Typography>
                                    </Fade>
                                    <Fade in timeout = {2000} style = {{transitionDelay: '400ms'}}>
                                        <Typography variant = 'body1' fontSize = '2vw' fontWeight = 'bold' fontStyle = 'italic' align = 'center' style = {{width: '75vw'}} sx = {{textShadow: '0px 0px 10px black'}} mt = '4.5%'>
                ...
                :
                    <Box style = {{width: width, height: height, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', alignItems: 'center', backgroundImage: `url(${data.image})`, backgroundSize: 'cover', backgroundPosition: 'center'}}>
                        <Box style = {{width: width, height: height, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', alignItems: 'center', backgroundColor: data.image ? 'rgba(0, 0, 0, 0.50)': '#171717', backdropFilter: 'blur(10px)'}}>
                            <Box style = {{width: width, height: height * 0.875, display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', color: 'white'}}>
                                <Box style = {{width: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'justify'}}>
                                    <Fade in timeout = {1600}>
                                        <Typography variant = 'h1' fontFamily = 'Segoe UI, sans-serif' fontSize = '5vh' fontWeight = 'regular' align = 'center' sx = {{textShadow: '0px 0px 10px rgba(23, 23, 23, 0.70)'}}>
                                            {data.title}
                                        </Typography>
                                    </Fade>
                                    <Fade in timeout = {2000} style = {{transitionDelay: '400ms'}}>
                                        <Typography variant = 'body1' fontSize = '2vh' fontWeight = 'bold' fontStyle = 'italic' align = 'center' style = {{width: width * 0.80}} sx = {{textShadow: '0px 0px 10px rgba(23, 23, 0.70)'}} mt = '9%'>
                                            {data.introduction.text.split("<LINK>").map((i, index) => (
                                                <React.Fragment key = {index}>
                                                    {index % 2 === 0 ?
                                                        i
                ...
                <Box style = {{width: windowWidth, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'justify', backgroundColor: 'white'}} pb = '6%'>
                    ...
                    <Box style = {{maxWidth: width >= 750 ? '70%' : '80%'}}>
                        <ul style = {{listStyle: 'square', fontFamily: 'Quicksand', fontSize: width >= 750 ? '1.5vw' : '1.5vh', margin: 0, padding: 0}}>
                            {data.common_symptoms.map((i, index) => (
                                <li key = {index}>
                    ...
                    <Box style = {{maxWidth: width >= 750 ? '70%' : '80%'}}>
                        <ul style = {{listStyle: 'square', fontFamily: 'Quicksand', fontSize: width >= 750 ? '1.5vw' : '1.5vh', margin: 0, padding: 0}}>
                            {data.potential_causes.map((i, index) => (
                                <li key = {index}>
                                    <Typography variant = 'body1' mt = '1.5%' mb = '1.5%'>
                                        <strong>{i.title}</strong>: {i.text}
                                    </Typography>
                                    ...
                                    <Typography variant = 'body1' mt = '1.5%' mb = '1.5%'>
                                        {Object.entries(i.title).sort((a, b) => a[0] > b[0] ? 1 : a[0] < b[0] ? -1 : 0).map((j, index_2) => (
                                            <React.Fragment key = {j[0]}>
                                                ...
                                                :
                                                    <Link to = {'/botanicals/' + j[1]} style = {{color: 'black', fontWeight: 'bold'}}>
                                                        {j[0]}
                                                    </Link>
                                                }
